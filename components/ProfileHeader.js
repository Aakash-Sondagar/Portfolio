...
export default function ProfileHeader({ view }) {
  ...
  const downloadResume = () => {
    ...
    anchorElement.href = profileHeader.ResumeLink[view];
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:justify-between items-center sm:items-center sm:space-y-0 animate-slideFromDownAndFade">
      <div className="flex items-center flex-col sm:flex-row">
        <Avatar profile={true} className="h-[70px] w-[70px] sm:h-16 sm:w-16 hover:scale-105 sm:mr-5">
          <AvatarImage
      ...
      <div className="flex gap-4 sm:gap-3 items-center justify-center font-semibold mt-3">
        ...
        >
          ...
          {download === "true" ? (
            <Check className="h-6 w-6 sm:h-5 sm:w-5 sm:ml-1 transition-all duration-300 ease-in-out opacity-100 transform scale-100" />
          ) : (
            <ArrowDownToLine className="h-6 w-6 sm:h-5 sm:w-5 sm:ml-1 transition-all duration-300 ease-in-out opacity-100 transform scale-100" />
          )}
        </Button>
